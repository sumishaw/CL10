...
object HindiTtsService {
    ...
    // Ordinary profanity Hindi renderings — matches what whisper_server.py's
    // _EN_TO_HI_DIRECT forces into the translation for fuck/shit/ass/bitch/
    // bastard/crap/bullshit. Deliberately does NOT include slurs or explicit
    // sexual vocabulary — this list only covers the words already agreed on.
    private val PROFANITY_RESPELL_WORDS = listOf(
        "चोदने वाला", "चोदने वाले", "चोदा हुआ",
        "चोदो", "चोदा", "चोदने", "चोदता", "चोदते",
        "भाड़",
        "कमीना", "कमीने",
        "गधा",
        "कुतिया",
    ...
    // Breaks an exact-string match against whatever word list the TTS
    // engine's internal content filter checks, by inserting a zero-width
    // non-joiner (U+200C) after the first character of each recognized word.
    // Devanagari text shaping still renders (and pronounces) essentially the
    // same either way — ZWNJ only suppresses ligature formation, it doesn't
    // drop or alter phonemes — but the resulting string no longer matches the
    // blocked entry, so the engine has nothing to mute.
    private fun respellProfanityForTts(text: String): String {
        var result = text
        for (word in PROFANITY_RESPELL_WORDS) {
            if (result.contains(word)) {
                val respelled = word.take(1) + "\u200C" + word.drop(1)
                result = result.replace(word, respelled)
            }
        }
        return result
    }
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (!outFile.exists() || outFile.length() < 100) {
                    CaptionLogger.log(TAG, "TTS output file empty — retrying", CaptionLogger.LEVEL_WARN)

                    // Step 1: if we used SSML, retry with plain text first — SSML
                    // markup itself can sometimes trip synthesis, independent of
                    // any content-filtering issue.
                    var recovered = false
                    ...
                    // Step 2: PROFANITY-RESPELL — an empty file even for plain
                    // text (result==SUCCESS, near-zero audio) is the signature
                    // of the TTS engine's own internal content filter silently
                    // muting a specific word, not a genuine synthesis error.
                    // Break the exact-string match it's likely checking for by
                    // inserting zero-width non-joiners inside known ordinary
                    // profanity words — Devanagari shaping still renders/speaks
                    // essentially the same, but the string no longer matches
                    // whatever blocklist entry triggered the mute.
                    if (!recovered) {
                        val respelled = respellProfanityForTts(safeText)
                        if (respelled != safeText) {
                            val respellId = UUID.randomUUID().toString()
                            val respellDeferred = CompletableDeferred<Unit>()
                            pendingUtterances[respellId] = { respellDeferred.complete(Unit) }
                            val respellResult = localTts.synthesizeToFile(respelled, null, outFile, respellId)
                            if (respellResult == TextToSpeech.SUCCESS) {
                                withTimeoutOrNull(8_000L) { respellDeferred.await() }
                            } else {
                                pendingUtterances.remove(respellId)
                            }
                            if (outFile.exists() && outFile.length() >= 100) {
                                CaptionLogger.log(TAG, "TTS-PROFANITY-RESPELL succeeded: '$safeText' → '$respelled'")
                                recovered = true
                            } else {
                    ...
                    if (!recovered) {
                        CaptionLogger.log(TAG, "TTS output still empty after all retries — skip")
                        return@withContext null
                    }
                }
