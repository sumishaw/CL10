...
object HindiTtsService {
    ...
    // CRITICAL FIX: was `private val scope` — a val can never be reassigned,
    // so once destroy() calls scope.cancel() (e.g. on a service stop/restart,
    // or the user stopping and restarting the Caption Lens session within
    // the same app process), this scope stays cancelled FOREVER. Any later
    // init() call still does scope.launch { fetchLoop(...) } on that dead
    // scope — which creates a Job that silently never executes its body, no
    // crash, no exception, no log line anywhere. That's exactly what
    // produced the "SPEAK keeps logging, TTS-WAV/PLAY never do" symptom:
    // the fetch worker coroutine was launched but never actually ran.
    // Now: a fresh scope is created every init() call (see below), so a
    // prior destroy() can never permanently break future sessions.
    ...
    fun init(context: Context) {
        // Fresh scope every init() — see the comment on the `scope` property
        // for why this matters: reusing a scope that a prior destroy() call
        // already cancelled would silently launch dead coroutines forever.
        if (!scope.isActive) {
    ...
    private suspend fun fetchLoop(workerName: String) {
        while (currentCoroutineContext().isActive) {
            ...
            try {
                ...
                // CRITICAL FIX: previously nothing in this loop body was
                // caught — any unexpected exception here would silently kill
                // this coroutine FOREVER (no crash, no log), leaving every
                // future speak() call queuing forever with nothing ever
                // synthesized or played. Now: log it and keep the worker
                // alive for the next item instead of dying silently.
                CaptionLogger.log(TAG, "FETCH-LOOP-EXC ${e.javaClass.simpleName}: ${e.message}", CaptionLogger.LEVEL_ERROR)
            }
        }
    }
    ...
    // Maps the REAL measured 10-point F0 contour (capturedF0Curve, filled by
    // GenderAnalyzer.flushContour() from the actual speaker's audio this
    // sentence) onto per-word SSML pitch — so the Hindi speech's melody
    // mirrors how the ORIGINAL speaker's pitch actually rose and fell,
    // rather than a flat average or a fixed emotion template. This is what
    // makes it sound like a continuation of THEIR voice, not just "a voice
    // with the right gender and rate."
    //
    // Smoothed, not applied raw: Android TTS's per-word <prosody pitch>
    // causes an audible robotic "staircase" jump when consecutive words
    // differ too much (see buildSsml's comments below — same constraint).
    // Each word's target pitch is capped to move at most MAX_STEP_PCT from
    // the previous word, easing toward the measured shape instead of
    ...
    private fun buildMelodyContour(text: String, curve: FloatArray, basePitchPct: String): String? {
        ...
        if (validPts.size < 3) return null   // not enough real signal captured this sentence
    ...
    private fun emotionSsml(text: String, emotion: Emotion, basePitchPct: String): String? {
        ...
        return when (emotion) {
            ...
            Emotion.HAPPY -> {
                // Light bouncy energy — mild alternating lift, not as extreme as LAUGHING
                val words = text.split(" ")
                val tagged = words.mapIndexed { i, w ->
                    val p = if (i % 2 == 0) "+14%" else "+8%"
                    "<prosody pitch='$p'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='medium-fast'>$tagged</prosody></speak>"
            }
            Emotion.SAD -> {
                // Gently drooping across the sentence — starts near baseline, drifts down
                val words = text.split(" ")
                val n = words.size.coerceAtLeast(1)
                val tagged = words.mapIndexed { i, w ->
                    val pct = -2 - (i * 16 / n)
                    "<prosody pitch='${pct}%'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='slow'>$tagged</prosody></speak>"
            }
            Emotion.ANGRY -> {
                // Clipped, hard emphasis, tight pitch band — sharper than COMMANDING
                val words = text.split(" ")
                val tagged = words.mapIndexed { i, w ->
                    if (i % 2 == 0)
                        "<emphasis level='strong'><prosody pitch='+6%'>${android.text.Html.escapeHtml(w)}</prosody></emphasis>"
                    else "<prosody pitch='-4%'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='fast'>$tagged</prosody></speak>"
            }
            Emotion.EXCITED -> {
                // Builds — energy climbs toward the end of the phrase
                val words = text.split(" ")
                val n = words.size.coerceAtLeast(1)
                val tagged = words.mapIndexed { i, w ->
                    val pct = 6 + (i * 24 / n)
            ...
            Emotion.CURIOUS -> {
                // Flat until a rising lilt on the last 1-2 words (question-like)
                val words = text.split(" ")
                val n = words.size
                val tagged = words.mapIndexed { i, w ->
                    val pct = if (i >= n - 2) "+20%" else "+2%"
                    "<prosody pitch='$pct'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='medium'>$tagged</prosody></speak>"
            }
            Emotion.WARM -> {
                // Soft, gently falling, reduced emphasis — caring but not tender-intimate
                "<speak><prosody pitch='-4%' rate='medium'><emphasis level='reduced'>$esc</emphasis></prosody></speak>"
            }
            Emotion.FEARFUL -> {
                // Trembling jitter — small rapid alternation, fast rate
                val words = text.split(" ")
                val tagged = words.mapIndexed { i, w ->
                    val p = if (i % 2 == 0) "+18%" else "+10%"
                    "<prosody pitch='$p'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='fast'>$tagged</prosody></speak>"
            }
            Emotion.SURPRISED -> {
                // Sharp spike on the first word/phrase, then settles
                val words = text.split(" ")
                val tagged = words.mapIndexed { i, w ->
                    val p = if (i == 0) "+38%" else if (i == 1) "+15%" else "+2%"
                    "<prosody pitch='$p'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='medium-fast'>$tagged</prosody></speak>"
            }
            Emotion.SIGHING -> {
                // Leading breath pause, then falling — slow throughout
                "<speak><break time='200ms'/><prosody pitch='-10%' rate='slow'>$esc</prosody></speak>"
            }
            Emotion.STRAINED -> {
                // Tight, narrow pitch band — tense, not much melodic movement
                "<speak><prosody pitch='+2%' rate='medium-slow' range='x-low'>$esc</prosody></speak>"
            }
            Emotion.GRAVELLY -> {
                // Low, rough register, slow-medium pace
                "<speak><prosody pitch='-14%' rate='medium-slow'>$esc</prosody></speak>"
            }
            Emotion.RASPY -> {
                // Similar to gravelly but slightly less low, a touch faster
                "<speak><prosody pitch='-10%' rate='medium'>$esc</prosody></speak>"
            }
            Emotion.MURMURED -> {
                // Quiet, reduced emphasis throughout, low pitch
                "<speak><prosody pitch='-12%' rate='slow'><emphasis level='reduced'>$esc</emphasis></prosody></speak>"
            }
            Emotion.HUSHED -> {
                // Very subdued — reduced emphasis, low pitch, small internal pause
                val mid = text.length / 2
                val part1 = text.take(mid).trim(); val part2 = text.drop(mid).trim()
                "<speak><prosody pitch='-15%' rate='slow'><emphasis level='reduced'>" +
                "${android.text.Html.escapeHtml(part1)}<break time='150ms'/>${android.text.Html.escapeHtml(part2)}" +
                "</emphasis></prosody></speak>"
            }
            Emotion.TENDER -> {
                // Gentle, warm, caring (e.g. comforting a child) — soft falling arc
                "<speak><prosody pitch='-6%' rate='slow'><emphasis level='reduced'>$esc</emphasis></prosody></speak>"
            }
            Emotion.DISGUST -> {
                // Sharp downward inflection with a clipped opening burst
                val words = text.split(" ")
                val tagged = words.mapIndexed { i, w ->
                    val p = if (i == 0) "+10%" else "-10%"
                    "<prosody pitch='$p'>${android.text.Html.escapeHtml(w)}</prosody>"
                }.joinToString(" ")
                "<speak><prosody rate='medium'>$tagged</prosody></speak>"
            }
            ...
            // sound robotic there. The real-melody engine is reserved for
            // actual non-neutral emotional moments, where added expressive
            // variation is a fairer tradeoff. Untemplated emotions (the
            // untouched MOANING/PANTING/BREATHY/SULTRY/HUSKY/WHISPERY/
            // VELVETY/GASPING cluster) fall through to the melody engine
            // here too, same as any other unhandled emotion — no special
            // treatment added for them specifically.
        }
    }
    ...
    // Strips characters that TTS engines tend to read aloud literally by
    // name ("at", "hash", "underscore"...) instead of treating as silent
    // punctuation. Devanagari/Latin letters, digits, and standard sentence
    // punctuation (। . , ? !) are kept — those just produce natural pauses,
    // they aren't spoken. Everything else (@ # $ % ^ & * _ ~ ` | \ [ ] { }
    // = + / and standalone hyphens) is stripped entirely, since these come
    // from caption/OCR artifacts or CT2 output noise, not real speech.
    private fun sanitizeForSpeech(text: String): String {
        val cleaned = text
            // Strip the noisy symbol set outright
            .replace(Regex("[@#$%^&*_~`|\\\\\\[\\]{}=+/]"), "")
            // Hyphens: drop when isolated/surrounded by spaces (list markers,
            // dash-artifacts) but keep genuine hyphenated-word hyphens intact
            .replace(Regex("(?<=\\s)-(?=\\s)|^-\\s*|\\s*-$"), " ")
            .replace(Regex("\\s{2,}"), " ")
            .trim()

        // If nothing but punctuation/whitespace is left (no actual letters
        // or digits in any script), there's nothing worth speaking at all —
        // this is what stops isolated ". ? !" fragments from being voiced.
        val hasContent = cleaned.any { it.isLetterOrDigit() }
        return if (hasContent) cleaned else ""
    }
    ...
    private suspend fun synthesizeToFileInner(item: FetchItem): File? =
        withContext(Dispatchers.IO) {
            ...
            val safeText = sanitizeForSpeech(item.text
                .replace("&", "and")
